#
# [Source:] Copied and adapted from [Airflow Sensor to check status of DAG Execution](https://medium.com/@sunilkhaire17/airflow-sensor-to-check-status-of-dag-execution-225342ec2897)
#
...
"""
This sensor is for checking latest status of dag.
If its success then only it will return true or mark
...
class DagStatusSensor(BaseSensorOperator):
    ...
    def poke (self, context: dict) -> None:

        dag_runs = DagRun.find(dag_id=self.dag_name)
        length = len(dag_runs)
        if dag_runs[length-1].state == self.status_to_check:
            return True
        elif dag_runs[length-1].state == State.FAILED:
            raise AirflowException(f"""Exception occcured and dag with dag_id '{self.dag_name}' failed""")
