...
class DownloadNOAADimFileToStagingOperator(BaseOperator):
    """ If a new version of the specified file exists
        ...
        To perform the check of most-recent-version on S3, AWS_KEY,
        AWS_SECRET and AWS_REGION need to be defined in the local
        environment. Otherwise, it is assumed that the most recent file is
        already on local staging, i.e. no download is triggered to avoid
