...
class DownloadOpenAQFilesOperator(BaseOperator):
    ...
    def execute(self, context: dict):
        ...
        for full_s3_filename in all_files:
            file_elements = full_s3_filename.split('/')
            # the actual file name is in the last list element
            full_local_filename = os.path.join(self.local_path, file_elements[-1])
            ...
            if not os.path.isfile(full_local_filename):
                ...
                self.log.info(f"Skipping download, file {full_local_filename} exists locally.")
